...
8. 程序是一个可执行文件，程序的实例称为进程，通常一个进程只有一个线程。但是你也可以创建多个线程。
	- 一个进程内的所有线程共享同一个地址空间、文件描述符、栈、进程相关属性
	- 多线程的程序需要在各线程访问共享数据时采取同步措施来避免不一致性
...
10. 登录口令在/etc/passwd文件中，由7个以冒号分隔的字段组成，分别为：登录名，加密口令，用户ID，组ID，注释字段，起始目录，shell程序。
